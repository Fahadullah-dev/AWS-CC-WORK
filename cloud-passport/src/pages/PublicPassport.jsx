      ...
      <div style={cardStyle}>
        ...
        <div style={{ padding: '30px' }}>
          <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', marginBottom: '30px', textAlign: 'center' }}>
            ...
            <div style={{ display: 'inline-flex', alignItems: 'center', gap: '8px', backgroundColor: 'black', color: '#FF9900', padding: '6px 14px', fontWeight: '900', fontSize: '14px', boxShadow: '4px 4px 0px rgba(0,0,0,0.2)', transform: 'rotate(-2deg)' }}>
              <img src={TIER_ICONS[currentTier]} alt={currentTier} style={{ width: '20px', height: '20px', objectFit: 'contain', filter: 'drop-shadow(0px 0px 2px #FF9900)' }} />
              ...
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: '10px' }}>
                {attendance.map(a => (
                  <div key={a.id} style={{ border: '3px solid black', padding: '8px 12px', fontSize: '12px', fontWeight: '900', backgroundColor: '#f9f9f9', display: 'flex', alignItems: 'center', gap: '8px', boxShadow: '3px 3px 0px rgba(0,0,0,0.1)' }}>
                    <img src={a.event?.emoji || "/icons/event-default.png"} style={{ width: '20px', height: '20px', objectFit: 'contain' }} alt="stamp" /> 
                    {a.event?.name}
                  </div>
                ))}
              </div>
...
// STYLES
const containerStyle = { minHeight: '100vh', display: 'flex', justifyContent: 'center', alignItems: 'flex-start', padding: '40px 20px', backgroundColor: '#f4f4f4', backgroundImage: 'radial-gradient(#d1d1d1 1px, transparent 1px)', backgroundSize: '20px 20px', color: 'black' };
